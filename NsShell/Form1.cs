...
namespace NsShell
{
     public partial class Form1 : Form
     {
          ...
          private object LoadItemsToListView(object filesToLoadobj)
          {
               ...
               {
                    item = new ListViewItem(new string[] { file.Name, file.Extension.ToUpper(), FormatByteSize(file), File.GetLastWriteTime(file.FullName).ToShortDateString() + " " + File.GetLastWriteTime(file.FullName).ToShortTimeString(), GetProgramType(file.Extension), @file.FullName }, count++);
                    ret.Add(item);
               });
